...
const PENDANT_URL = "http://localhost:8000/api/game-info/server-information/pendant-information";
const GEM_URL = "http://localhost:8000/api/game-info/server-information/gem-information";
const SERVERS_INFO_URL = "http://localhost:8000/api/game-info/server-information/serversinfo";
const SYSTEM_INFO_URL = "http://localhost:8000/api/game-info/server-information/systeminfo";
const FEATURE_DISABLE_URL = "http://localhost:8000/api/game-info/server-information/feature-disable";
const FEATURE_ENABLE_URL = "http://localhost:8000/api/game-info/server-information/feature-enable";
const NPC_LIST_URL = "http://localhost:8000/api/game-info/server-information/npclist";
const DROP_LIST_URL = "http://localhost:8000/api/game-info/server-information/droplist";
...
export default function ServerInfo() {
        ...
        return (
          <div className="w-full text-sm flex flex-col gap-6">
            {data.serversInfo && data.serversInfo.length > 0 && (
              <div>
                <h3 className="font-bold text-center text-lg border-b-2 border-yellow-400 pb-1 mb-2">SERVER INFORMATION</h3>
                <div className="flex flex-col">
                  {data.serversInfo.map((item, index) => (
                    <div key={`server-${index}`} className="flex justify-between py-2 border-b border-gray-800 last:border-b-0">
            ...
            {data.systemInfo && data.systemInfo.length > 0 && (
              <div>
                <h3 className="font-bold text-lg text-center border-b-2 border-yellow-400 pb-1 mb-2">SYSTEM INFORMATION</h3>
                <div className="flex flex-col">
                  {data.systemInfo.map((item, index) => (
                    <div key={`system-${index}`} className="flex justify-between py-2 border-b border-gray-800 last:border-b-0">
            ...
            {data.featureEnable && data.featureEnable.length > 0 && (
              <div>
                <h3 className="font-bold text-lg border-b-2 border-yellow-400 pb-1 mb-2">ENABLED FEATURES</h3>
                <div className="flex flex-col">
                  {data.featureEnable.map((item, index) => (
                    <div key={`enable-${index}`} className="flex justify-between py-2 border-b border-gray-800 last:border-b-0">
            ...
            {data.featureDisable && data.featureDisable.length > 0 && (
              <div>
                <h3 className="font-bold text-lg border-b-2 border-yellow-400 pb-1 mb-2">DISABLED FEATURES</h3>
                <div className="flex flex-col">
                  {data.featureDisable.map((item, index) => (
                    <div key={`disable-${index}`} className="flex justify-between py-2 border-b border-gray-800 last:border-b-0">
                      <span className="font-semibold">{item.feature}</span>
                      <span>{item.description}</span>
                    </div>
                  ))}
            ...
            <div className="w-full text-sm">
              ...
              {data.map((item, index) => (
                <div key={index} className="flex p-2 border-b border-gray-700 last:border-b-0">
                  ...
                  <div className="w-1/3">{item.map_information?.name || item.map_information_id}</div>
  ...
  if (loading) return <div className="flex justify-center items-center h-screen text-white">Loading Server Information...</div>;
      ...
      <div className="bg-cover bg-no-repeat main-background-container">
              ...
              <div key={index} className="w-full">
                ...
                {openIndex === index && (
                  <div className="pl-6 pr-4 py-4 text-gray-300">
                    {renderContent(section.dataKey, serverData[section.dataKey])}
                  </div>
                )}
        ...
        <div className="items-center justify-center pb-4">
          ...
          <div className="flex flex-row justify-center items-center gap-2">
            <a href="https://www.instagram.com/rfvikings" target="_blank" rel="noopener noreferrer" className="hover:text-pink-400 transition"><FaInstagram /></a>
            <a href="https://tiktok.com/@rfvikings" target="_blank" rel="noopener noreferrer" className="hover:text-black transition"><FaTiktok /></a>
            <a href="https://www.facebook.com/profile.php?id=61562554693454" target="_blank" rel="noopener noreferrer" className="hover:text-blue-500 transition"><FaFacebook /></a>
            <a href="https://discord.gg/rfvikings" target="_blank" rel="noopener noreferrer" className="hover:text-indigo-400 transition"><FaDiscord /></a>
          </div>
        </div>
      </div>
    </section>
