...
const DailyQuestWednesday = () => {
  const [dailyQuestWednesdays, setDailyQuestWednesdays] = useState([]);
  ...
  const formFields = [
    ...
    { label: 'Tutorial', name: 'tutorial', type: 'textarea', required: true, placeholder: 'Enter tutorial steps, each on a new line. Example:\n1. Check daily login bonus\n2. Complete a dungeon' }, 
    { label: 'Quest', name: 'quest', type: 'textarea', required: false, placeholder: 'Enter quest objectives, each on a new line. Example:\n- Collect 5 rare herbs\n- Defeat the forest guardian' }, 
    { label: 'Reward', name: 'reward', type: 'textarea', required: false, placeholder: 'Enter rewards, each on a new line. Example:\n400 Mana\n1x Epic Chest' }, 
  ];

  useEffect(() => {
    fetchDailyQuestWednesdays();
  }, []);

  const fetchDailyQuestWednesdays = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/game-info/quest-information/dailyquestwednesday');
      if (!response.ok) {
          throw new Error('Gagal mengambil data Daily Quest Wednesday: ' + response.statusText);
      }
      const data = await response.json();
      setDailyQuestWednesdays(data);
    } catch (error) {
      console.error('Error fetching Daily Quest Wednesdays:', error);
      setToastMessage(`Gagal mengambil data Daily Quest Wednesday: ${error.message}`);
      setShowToast(true);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = currentItem
        ? `http://127.0.0.1:8000/api/game-info/quest-information/dailyquestwednesday/${currentItem.id}`
        : 'http://127.0.0.1:8000/api/game-info/quest-information/dailyquestwednesday';
      ...
      if (response.ok) {
        fetchDailyQuestWednesdays();
        handleCloseModal();
        setToastMessage(currentItem ? 'Daily Quest Wednesday berhasil diperbarui.' : 'Daily Quest Wednesday berhasil ditambahkan.');
        setShowToast(true);
      } else {
      ...
      console.error('Error submitting Daily Quest Wednesday:', error);
      setToastMessage(`Gagal menyimpan Daily Quest Wednesday: ${error.message}`);
      setShowToast(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/game-info/quest-information/dailyquestwednesday/${id}`, { method: 'DELETE' });
        ...
        if (response.ok) {
          fetchDailyQuestWednesdays();
          setToastMessage('Daily Quest Wednesday berhasil dihapus.');
          setShowToast(true);
        } else {
        ...
        console.error('Error deleting Daily Quest Wednesday:', error);
  ...
  return (
    <div className="container mt-3">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="mb-0">Daily Quest Wednesday</h2>
        <Button variant="primary" onClick={handleShowModal}>
          Tambah Daily Quest Wednesday
        </Button>
      </div>
              ...
              <td>
                {Array.isArray(item.image) && item.image.length > 0 ? (
                  item.image.map((imgSrc, idx) => (
                    <Image
                      key={idx}
                      src={`http://127.0.0.1:8000/storage/dailyquestwednesday/${imgSrc}`}
                      alt={`Quest Image ${idx}`}
                      thumbnail
                      width="50"
                      height="50"
                  ...
                  <Image
                    src={`http://127.0.0.1:8000/storage/dailyquestwednesday/${item.image}`}
                    alt="Quest Image"
                    thumbnail
                    width="50"
                    height="50"
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>{currentItem ? 'Edit Daily Quest Wednesday' : 'Tambah Daily Quest Wednesday'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default DailyQuestWednesday;