...
const DailyQuest = () => {
  ...
  const formFields = [
    {
      ...
      type: "text",
      required: true,
      placeholder: "Masukkan Hari (misalnya, Senin, Setiap Hari)",
    },
    {
      label: "Image",
      name: "image",
    ...
    },
    {
      ...
      type: "textarea",
      required: true,
      placeholder: "Masukkan langkah-langkah tutorial atau deskripsi lengkap.",
    },
    {
      label: "Quest",
      name: "quest",
  ...
  const fetchDailyQuests = async () => {
    try {
      const response = await api.get("/game-info/quest-information/dailyquest");
      setDailyQuests(response.data);
    } catch (error) {
      const msg = error.response?.data?.message || error.message;
      setToastMessage(`Gagal mengambil data Daily Quest: ${msg}`);
      setToastType("error");
      setShowToast(true);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = currentItem
        ? `/game-info/quest-information/dailyquest/${currentItem.id}`
      ...
      handleCloseModal();
      setToastMessage(
        currentItem ? "Berhasil diperbarui." : "Berhasil ditambahkan."
      );
      ...
      const errMsg =
        ...
        Object.values(error.response?.data?.errors || {})
          ...
          .join("; ") ||
        error.message;
      setToastMessage(`Gagal menyimpan: ${errMsg}`);
      setToastType("error");
      setShowToast(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/game-info/quest-information/dailyquest/${id}`);
      fetchDailyQuests();
                  ...
                  {typeof item.image === "string" && item.image ? (
                    <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.src =
                          "https://placehold.co/48x48/e0e0e0/555?text=No+Img";
                      }}
                    />
                  ) : (
