...
const DailyQuestMonday = () => {
  const [dailyQuestMondays, setDailyQuestMondays] = useState([]);
  ...
  const formFields = [
    ...
    { label: 'Tutorial', name: 'tutorial', type: 'textarea', required: true, placeholder: 'Enter tutorial steps, each on a new line. Example:\n1. Collect resources\n2. Craft item' }, // Updated placeholder
    { label: 'Quest', name: 'quest', type: 'textarea', required: false, placeholder: 'Enter quest objectives, each on a new line. Example:\n- Defeat 3 monsters\n- Find a hidden artifact' }, // Updated placeholder
    { label: 'Reward', name: 'reward', type: 'textarea', required: false, placeholder: 'Enter rewards, each on a new line. Example:\n300 Gold\n1x Common Potion' }, // Updated placeholder
  ];

  useEffect(() => {
    fetchDailyQuestMondays();
  }, []);

  const fetchDailyQuestMondays = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/game-info/quest-information/dailyquestmonday');
      if (!response.ok) {
          throw new Error('Gagal mengambil data Daily Quest Monday: ' + response.statusText);
      }
      const data = await response.json();
      setDailyQuestMondays(data);
    } catch (error) {
      console.error('Error fetching Daily Quest Mondays:', error);
      setToastMessage(`Gagal mengambil data Daily Quest Monday: ${error.message}`);
      setShowToast(true);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = currentItem
        ? `http://127.0.0.1:8000/api/game-info/quest-information/dailyquestmonday/${currentItem.id}`
        : 'http://127.0.0.1:8000/api/game-info/quest-information/dailyquestmonday';
      ...
      if (response.ok) {
        fetchDailyQuestMondays();
        handleCloseModal();
        setToastMessage(currentItem ? 'Daily Quest Monday berhasil diperbarui.' : 'Daily Quest Monday berhasil ditambahkan.');
        setShowToast(true);
      } else {
      ...
      console.error('Error submitting Daily Quest Monday:', error);
      setToastMessage(`Gagal menyimpan Daily Quest Monday: ${error.message}`);
      setShowToast(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/game-info/quest-information/dailyquestmonday/${id}`, { method: 'DELETE' });
        ...
        if (response.ok) {
          fetchDailyQuestMondays();
          setToastMessage('Daily Quest Monday berhasil dihapus.');
          setShowToast(true);
        } else {
        ...
        console.error('Error deleting Daily Quest Monday:', error);
  ...
  return (
    <div className="container mt-3">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="mb-0">Daily Quest Monday</h2>
        <Button variant="primary" onClick={handleShowModal}>
          Tambah Daily Quest Monday
        </Button>
      </div>
              ...
              <td>
                {Array.isArray(item.image) && item.image.length > 0 ? (
                  item.image.map((imgSrc, idx) => (
                    <Image
                      key={idx}
                      src={`http://127.0.0.1:8000/storage/dailyquestmonday/${imgSrc}`}
                      alt={`Quest Image ${idx}`}
                      thumbnail
                      width="50"
                      height="50"
                  ...
                  <Image
                    src={`http://127.0.0.1:8000/storage/dailyquestmonday/${item.image}`}
                    alt="Quest Image"
                    thumbnail
                    width="50"
                    height="50"
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        <Modal.Header closeButton>
          <Modal.Title>{currentItem ? 'Edit Daily Quest Monday' : 'Tambah Daily Quest Monday'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default DailyQuestMonday;