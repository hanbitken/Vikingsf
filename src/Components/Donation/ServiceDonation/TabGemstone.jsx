...
const TabGemstone = () => {
  ...
  const fetchGemstones = async () => {
    try {
      const response = await api.get('/donation/service/gemstone');
      setGemstones(response.data);
    } catch (error) {
      setToastMessage(`Failed to fetch Gemstone data: ${error.response?.data?.message || error.message}`);
      setToastType('error');
      setShowToast(true);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentItem) {
        await api.put(`/donation/service/gemstone/${currentItem.id}`, formData);
        setToastMessage('Gemstone data updated successfully.');
      } else {
        await api.post('/donation/service/gemstone', formData);
        setToastMessage('Gemstone data added successfully.');
      }
      fetchGemstones();
      handleCloseModal();
      ...
      setToastMessage(`Failed to save Gemstone data: ${error.response?.data?.message || error.message}`);
      setToastType('error');
      setShowToast(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/donation/service/gemstone/${id}`);
      fetchGemstones();
      setToastMessage('Gemstone data deleted successfully.');
      setToastType('success');
      setShowToast(true);
    } catch (error) {
      setToastMessage(`Failed to delete Gemstone data: ${error.response?.data?.message || error.message}`);
      setToastType('error');
      setShowToast(true);
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 max-w-5xl">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-gray-800 mb-0">Gemstone Data</h2>
        <button
