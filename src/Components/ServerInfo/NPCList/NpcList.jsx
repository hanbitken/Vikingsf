...
const NpcList = () => {
  ...
  const formFields = [
    ...
    { label: 'NPC', name: 'npc', type: 'text', required: true, placeholder: 'Enter NPC Name' },
  ...
  const fetchNpcLists = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/api/game-info/server-information/npclist');
      ...
      if (!response.ok) {
        ...
        throw new Error(`Gagal mengambil data NPC List: Status ${response.status} - ${errorText}`);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = currentItem
        ? `http://127.0.0.1:8000/api/game-info/server-information/npclist/${currentItem.id}`
        : 'http://127.0.0.1:8000/api/game-info/server-information/npclist';
      ...
      if (response.ok) {
        fetchNpcLists();
        handleCloseModal();
        setToastMessage(currentItem ? 'NPC List berhasil diperbarui.' : 'NPC List berhasil ditambahkan.');
        setToastType('success');
        setShowToast(true);
      } else {
      ...
      setToastMessage(`Gagal menyimpan NPC List: ${error.message}`);
      setToastType('error');
      setShowToast(true);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/game-info/server-information/npclist/${id}`, {
          method: 'DELETE',
        });
